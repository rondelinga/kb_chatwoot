...
const emit = defineEmits(['previous', 'next', 'reset', 'scaleChange']);
...
const displaySegments = computed(() =>
  props.segments.map(segment => {
    ...
    const left =
      ((segStart - props.timelineStart) / timelineDuration.value) * 100;
  ...
  <div class="rounded-xl border border-n-weak bg-n-alpha-2 p-4">
    <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-1.5">
        <select
          :value="selectedScale"
          class="min-w-max h-7 m-0 px-2 rounded-lg border border-n-weak bg-n-solid-1 text-n-slate-12 text-xs leading-7 outline-none focus:border-n-brand dark:bg-n-solid-2 appearance-none box-border align-middle"
          @change="emit('scaleChange', $event.target.value)"
        >
          <option
    ...
    >
      ...
      >
        ...
        >
          ...
          <div class="mt-0.5 text-[11px] font-medium text-n-slate-12">
    ...
    <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-n-slate-11">
      <span class="flex items-center gap-1.5">
        ...
        <span class="font-semibold text-emerald-600 dark:text-emerald-400">{{
          formatDuration(displayTotals.accepting)
        }}</span>
      </span>
      <span class="flex items-center gap-1.5">
        ...
        <span class="font-semibold text-amber-600 dark:text-amber-400">{{
          formatDuration(displayTotals.not_accepting)
        }}</span>
      </span>
      <span class="flex items-center gap-1.5">
        ...
        <span class="font-semibold text-slate-500 dark:text-slate-400">{{
          formatDuration(displayTotals.logged_out)
        }}</span>
      </span>
    </div>
  </div>
