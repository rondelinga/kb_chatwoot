...
const contactFields = computed(() => {
  ...
  return [
    ...
    {
      icon: 'i-ph-info',
      label: t('CONTACT_PANEL.DESCRIPTION'),
      value: description,
    },
  ].filter(f => f.value);
});
...
const socialFields = computed(() => {
  const profiles =
    props.selectedContact?.additionalAttributes?.socialProfiles || {};
  const icons = {
...
<template>
  <div class="flex flex-col items-start gap-6 pb-6">
    <div
      class="w-full flex flex-col gap-1 rounded-xl border border-n-weak bg-n-solid-2 p-4"
    >
      <div
      ...
      >
        <span class="size-4 text-n-slate-10 shrink-0" :class="field.icon" />
        <span class="text-sm text-n-slate-11 w-28 shrink-0">{{
          field.label
        }}</span>
    ...
    <div
      v-if="socialFields.length > 0"
      class="w-full flex flex-col gap-1 rounded-xl border border-n-weak bg-n-solid-2 p-4"
    >
      <h6
        class="text-xs font-medium text-n-slate-10 uppercase tracking-wide mb-2"
      >
    ...
    <Policy :permissions="['administrator']">
      <div
        class="flex flex-col items-start w-full gap-4 pt-4 border-t border-n-strong"
      >
