...
describe('useBranding', () => {
  ...
  describe('replaceInstallationName', () => {
    it('should replace "Qodeq" with installation name when both text and installation name are provided', () => {
      const { replaceInstallationName } = useBranding();
      const result = replaceInstallationName('Welcome to Qodeq');

      expect(result).toBe('Welcome to MyCompany');
    });

    it('should replace multiple occurrences of "Qodeq"', () => {
      const { replaceInstallationName } = useBranding();
      const result = replaceInstallationName(
        'Qodeq is great! Use Qodeq today.'
      );
    ...
    it('should return original text when installation name is not provided', () => {
      ...
      expect(result).toBe('Welcome to Qodeq');
    });
    ...
    it('should handle text without "Qodeq" gracefully', () => {
      const { replaceInstallationName } = useBranding();
    ...
    it('should be case-sensitive for "Qodeq"', () => {
      const { replaceInstallationName } = useBranding();
      const result = replaceInstallationName('Welcome to Qodeq and Qodeq');

      expect(result).toBe('Welcome to Qodeq and Qodeq');
    });
